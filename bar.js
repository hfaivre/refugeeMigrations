...
function drawBar(data, dataType, country) {
	...
	data.forEach(row=>{
		formattedData.push({
			...
			refugeeByCountryOfAsylum : +row['refugeeByCountryOfAsylum'],
			refugeeByCountryOfOrigin : +row['refugeeByCountryOfOrigin'],
			year: +row.year
		});
	});
    ...
    var countryData = formattedData.filter(d => d.country === country)
	...
	var axisLabel = dataType === "refugeeByCountryOfAsylum" ?
    `Refugees 
